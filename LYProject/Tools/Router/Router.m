...
+(NSError *)routerTo:(NSString *)url{
    return [[Router shareInstance]routerTo:url];
}

+(void)setRouterResultDeleget:(id<RouterResultDelegate>)delegate{
    [Router shareInstance].delegate = delegate;
}

+(BOOL)sendMsg:(NSString *)url object:(id)object{
    if (url) {
        [[Router shareInstance]sendMsg:url object:object];
        return YES;
    }
    return NO;
}
...
+(BOOL)observerMsg:(NSString *)url observerBlock:(RouterObserverMsgBlock)observerBlock{
    if (url || observerBlock) {
        [[Router shareInstance]observerMsg:url observerBlock:observerBlock];
        return YES;
    }
    return NO;
}
...
#pragma clang diagnostic pop
    ...
    if (url && [url hasPrefix:@"http"]) {
        ...
        NSString *protocol = [Router parseProtocolFromUrl:url];
        ...
        if (clsString) {
            ...
            NSDictionary *param = [Router parseParamFromUrl:url];
...
#pragma clang diagnostic pop
                if (controller && [controller isKindOfClass:[UIViewController class]]) {
                    ...
                    model.url = url;
                    model.param = param;
                    model.protocol = [Router parseProtocolFromUrl:url];
