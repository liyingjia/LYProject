...
@interface LYMenuAction()
@property (nonatomic) NSString      *title;
@property (nonatomic) UIImage       *image;
@property (copy, nonatomic)void (^handler)(LYMenuAction *);
@end

@implementation LYMenuAction
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(LYMenuAction *))handler{
    LYMenuAction *action = [[LYMenuAction alloc]initWithTitle:title image:image handler:handler];
    return action;
}
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(LYMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
...
@interface LYMenuView()<UITableViewDelegate,UITableViewDataSource>
{
    CGPoint          _refPoint; //点击点的位置
    UIView          *_refView; //view
    CGFloat          _menuWidth; //宽度
...
@property(nonatomic,copy) NSArray<LYMenuAction *> *actions;
@property(nonatomic,strong)UITableView  *tableView;
@property(nonatomic,strong)UIView  *contentView;
@property(nonatomic,strong)UIView  *bgView;
@end

static NSString *const menuCellID = @"LYMenuCell";
...
+ (instancetype)menuWithActions:(NSArray<LYMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    ...
    LYMenuView *menu = [[LYMenuView alloc]initWithActions:actions width:width atPoint:point];
    return menu;
}
...
+ (instancetype)menuWithActions:(NSArray<LYMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    ...
    LYMenuView *menu = [[LYMenuView alloc]initWithActions:actions width:width relyonView:view];
    return menu;
}

- (instancetype)initWithActions:(NSArray<LYMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    if (self = [super init]) {
...
- (instancetype)initWithActions:(NSArray<LYMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    if (self = [super init]) {
...
- (void)calculateArrowAndFrame{
    ...
    //设置出menu的x和y （默认情况）---点的位置的x坐标-menuWidth宽度的一半
    ...
    if (originX + width > kScreenWidth - 10) {
        ...
        //向左的情况间距也至少是kDefaultMargin
        originX = kDefaultMargin;
    }
    
    //设置三角形左边第一个角的起始点---相对于父view
...
- (CAShapeLayer *)drawMaskLayer{
    ...
    //画圆弧----顺时针最后边开始，依次为：0*M_PI，0.5*M_PI，1*M_PI，1.5*M_PI
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    LYMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:menuCellID forIndexPath:indexPath];
    LYMenuAction *action = _actions[indexPath.row];
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[LYMenuCell class] forCellReuseIdentifier:menuCellID];
    }
    return _tableView;
}
- (UIView *)bgView{
